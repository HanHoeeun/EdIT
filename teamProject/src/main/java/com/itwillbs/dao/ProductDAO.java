...
public class ProductDAO {
	...
	public void updateProduct(ProductDTO productDTO) {
		try {
			...
			// 3 sql "update products set p_title=?, p_type=?, p_price=?, p_detail=?,
			// p_date=?, p_status=?, p_file=? where p_num=?";
...
//				String sql="select * from board order by num desc";
	...
	public int getProductCount() {
		...
		try {
			...
			String sql = "select count(p_num) from products;";
			pstmt = con.prepareStatement(sql);
			// 4 실행 => 결과저장
			rs = pstmt.executeQuery();
			// 5 결과 행접근 => 열접근 => count변수 저장
			if (rs.next()) {
				p_count = rs.getInt("count(p_num)");
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public int getlaptopCount(ProductPageDTO ppageDTO) {
		...
		try {
			...
			// 3 sql select count(*) from board
			String sql = "select count(*) from products where p_type=?;";

			pstmt = con.prepareStatement(sql);

			pstmt.setString(1, "laptop");// 시작행-1
			// 4 실행 => 결과저장
			rs = pstmt.executeQuery();
	...
	public int gettabletCount(ProductPageDTO ppageDTO) {
		...
		try {
			...
			pstmt.setString(1, "tablet");// 시작행-1
			// 4 실행 => 결과저장
			rs = pstmt.executeQuery();
